...
def load_zerodha_option_symbols(spot_price, selected_month, selected_year):
...
try:
    should_refresh = int(vyapak_input[vyapak_input['name'] == 'should_refresh_zerodha']['value'].values[0])
    if should_refresh != 1:
    ...
    zerodha_options_symbols = load_zerodha_option_symbols(future_price, selected_month, selected_year)
    ...
    zerodhaStrikes = pd.DataFrame(zerodha_data_array)
    vyapak_input['value'][vyapak_input['name'] == 'should_refresh_zerodha'] = 0
    print("All zerodha data updated!!!!!" + str(len(zerodha_data_array)))
    return vyapak_input, zerodhaStrikes
