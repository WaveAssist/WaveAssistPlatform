...
import {
  ...
  createProjectAPI,
  deleteProjectApi,
} from "../services/all_projects_services";
  ...
  const fetchData = async () => {
    try {
      ...
      localStorage.setItem(
        "projects_array",
        JSON.stringify(data.project_array)
      );
    } catch (error) {
      console.error("FetchAllProjects failed:", error);
      showToast(
        "Something went wrong with loading projects, please try again.",
        "danger"
      );
    }
  };
    ...
    <div className="base_component">
      <div className="dashboard-header row">
        ...
        <div className="col-md-4 d-flex justify-content-end fixed-right align-items-center">
          <button
            className="btn btn-outline-secondary logout_button"
        ...
        <div className="content projects-row">
          ...
          <div className="row ">
            ...
            {projectArray.map((project: any) => (
              <div
                className="col-sm-4 project-card ml-5"
                key={project.project_key}
                onClick={() => handleViewDetails(project.project_key)}
              >
                <div className="card text-white bg-dark mb-3 ml-5 mr-5">
                  <div className="card-body position-relative p-3">
                    <h5 className="card-title translucent_white fs-4">
                      {project.name}
                    </h5>
                    <h5 className="card-title translucent_white_more fs-6 mb-3">
                      {project.project_key}
                    </h5>
                    <button
          ...
          <div className="mb-3">
            ...
            {showAlert && (
              <Alert variant="danger">
                Could not create the project, try a different name.
              </Alert>
            )}
          </div>
        </Modal.Body>
