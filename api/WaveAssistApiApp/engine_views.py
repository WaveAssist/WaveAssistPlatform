...
def load_flows_data(request):
    ...
    try:
        flows_array = project_object.flows_set.filter(running_status=1)
        flows_dict_array = []
        for flow_object in flows_array:
            flows_dict_array.append(flow_object.get_dict())
        output_dictionary = {'flows_array': flows_dict_array}
        return ResponseParser.getParsedSuccessMessage(output_dictionary, '200', 'Flow details loaded successfully.')
    except Exception as e:
        return ResponseParser.getParsedErrorMessage('Something went wrong: ' + str(e))
