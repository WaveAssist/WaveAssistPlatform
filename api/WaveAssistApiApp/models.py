...
class Account(models.Model):
    ...
    def get_dict(self):
        ...
        account_dict["pip_requirements_array_json"] = json.loads(
            str(self.pip_requirements_array_json)
        )
...
class AccessProvided(models.Model):
    ...
    project_access_type = models.IntegerField(
        default=0
    )  ##0 is nothing, 1 is read, 2 is write, 3 is admin  --> This is manage project from admin panel.
    data_run_access_type = models.IntegerField(
        default=0
    )  ##0 is nothing, 1 is read, 2 is write, 3 is admin  --> This is manage dataRun from admin panel.
    dashboard_access_type = models.IntegerField(
        default=0
    )  ##0 is nothing, 1 is read, 2 is edit  --> This is to view & edit dashboard
    created_at = models.DateTimeField(auto_now_add=True)
...
class Nodes(models.Model):
    ...
    output_data_key_array = models.ManyToManyField(
        "DataKey", related_name="output_data_array", blank=True
    )
    ...
    def get_dict_info_safe(self):
        node_dict = {}
        node_dict["id"] = self.id
        node_dict["name"] = self.name
        node_dict["node_key"] = self.node_key
        node_dict["python_code"] = ""
        node_dict["is_starting_node"] = self.is_starting_node
        node_dict["is_enabled"] = self.is_enabled
        node_dict["schedule_type"] = self.schedule_type
        node_dict["interval_schedule"] = str(self.interval_schedule)
    ...
    def get_dict_safe(self):
        node_dict = self.get_dict_info_safe()
        run_after_nodes_array = []
        for run_after_node in self.run_after_nodes_array.all().order_by(
            Lower("node_key")
        ):
            run_after_nodes_array.append(run_after_node.get_dict_info_safe())
...
class DashboardSection(models.Model):
    ...
    display_type = models.IntegerField(
        default=0
    )  ##0 is Table, 1 is Numbers, 2 is Graph, 3 is HTML
    ...
    def save(self, *args, **kwargs):
        if not self.dashboard_section_key:
            self.dashboard_section_key = (
                self.project_object.project_key + "-" + str(self.id)
            )
...
class DAG(models.Model):
    ...
    parent_deployment = models.ForeignKey(
        "Deployments", on_delete=models.CASCADE, null=True
    )
    periodic_task = models.ForeignKey(
        "django_celery_beat.PeriodicTask", on_delete=models.CASCADE, null=True
    )
    is_running = models.BooleanField(default=True)
    ...
    interval_schedule = models.ForeignKey(
        "django_celery_beat.IntervalSchedule", on_delete=models.PROTECT, null=True
    )
    crontab_schedule = models.ForeignKey(
        CrontabSchedule, null=True, blank=True, on_delete=models.CASCADE
    )
...
class DagRuns(models.Model):
    ...
    data_run_object = models.ForeignKey(
        "DataRuns", on_delete=models.CASCADE, null=True
    )  # Data run associated with this DAG run
    created_at = models.DateTimeField(auto_now_add=True)
