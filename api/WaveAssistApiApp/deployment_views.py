...
def generate_dag_image(request):
    ...
    if success:
        ...
        return ResponseParser.getParsedSuccessMessage(
            output_dict, "200", "Successfully uploaded image to S3"
        )
    else:
...
def deploy_project(request):  ##TCW
    ...
    success, message, user_object, data_run_object = (
        validator.validate_user_and_data_run(request, ADMIN_GTE)
    )
    if not success:
        return ResponseParser.getParsedErrorMessage(message)

    all_nodes = project_object.nodes_set.filter(is_enabled=True)
    starting_nodes = all_nodes.filter(is_starting_node=True, is_enabled=True).exclude(
        schedule_type="none"
    )
    ...
    if len(starting_nodes) == 0:
        return ResponseParser.getParsedErrorMessage(
            "No enabled starting nodes found in the project to Deploy.."
        )

    ##Check if the version is unique
    version = request.POST.get("version", "1.0.0")
    all_deployments = Deployments.objects.filter(
        project_object=project_object, version=version, data_run_object=data_run_object
    )
    if len(all_deployments) > 0:
        return ResponseParser.getParsedErrorMessage(
            "Version already exists for the project. Please provide a unique version."
        )
    ...
    all_deployments = Deployments.objects.filter(
        project_object=project_object, is_running=True, data_run_object=data_run_object
    )
    for deployment in all_deployments:
        try:
            ...
            return ResponseParser.getParsedErrorMessage(
                "Error in stopping the already running project: " + str(e)
            )
    ...
    try:
        with transaction.atomic():
            deployment_key = (
                "Deployment_" + version + "_" + data_run_object.data_run_key
            )
        ...
        return ResponseParser.getParsedErrorMessage(
            "Error in deploying the project: " + str(e)
        )
    ...
    return ResponseParser.getParsedSuccessMessage(
        output_dict, "200", "Successfully deployed the project"
    )


def stop_deployment(request):  ##TCW
    success, message, user_object, deployment_object = (
        validator.validate_user_and_deployment(request, ADMIN_GTE)
    )
    if not success:
        return ResponseParser.getParsedErrorMessage(message)
    try:
        ...
        return ResponseParser.getParsedErrorMessage(
            "Error in stopping the project: " + str(e)
        )

    output_dict = {"deployment": deployment_object.get_dict()}

    return ResponseParser.getParsedSuccessMessage(
        output_dict, "200", "Successfully stopped the deployment."
    )
...
def run_code(request: object) -> JsonResponse:
    ...
    return ResponseParser.getParsedSuccessMessage(
        output_dict, "200", "Successfully ran the code"
    )
...
def email_webhook(request):
    ...
    try:
        ...
        try:
            ...
            return ResponseParser.getParsedErrorMessage(
                "Project, DataRun, or Node not found."
            )
